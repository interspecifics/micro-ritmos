...
class stateMachine:
	...
	def start_note(self, i, l):
                route = "/raspi2/sinte(%d)/channelOut(%d)" %  (self.rPi_id, int(i/4),int(l))
		msg = OSC.OSCMessage()
	...
	def stop_note(self, i, l, t):
                route = "/raspi2/sinte(%d)/tiempo(%d)/channelOut(%d)" %  (self.rPi_id, int(i/4),t,l)
		msg = OSC.OSCMessage()
		msg.setAddress(route)
		msg.append(i)
		cOsc.send(msg)
